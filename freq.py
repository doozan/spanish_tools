...
def build_wordlist():
    ...
    for k, item in sorted(
        freq.items(), key=lambda item: (item[1]["count"], item[0]), reverse=True
    ):
        pos, word = k.split(":")
    ...
    for word, all_pos in repeatusage.items():
        ...
        for pos, count in all_pos.items():
            if count < (best_count / 2):
                wordlist[pos + ":" + word]["flags"].append(
...
for k, item in sorted(
    wordlist.items(), key=lambda item: (item[1]["count"], item[1]["word"]), reverse=True
):
