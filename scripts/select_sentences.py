...
parser.add_argument(
    "--outfile",
    default="spanish_data/sentences.forced",
    help="File to save sentence selections",
)
...
class SentenceChooser:
    ...
    def update_list(self):
        for idx in range(len(self._all_options)):
            ...
            checkbox.set_label("\n  ".join([c[0], c[1]]))
            checkbox.set_state(
                f"{c[3]}:{c[4]}" in self._selected_items, do_callback=False
            )
    ...
    def __init__(self, word, pos, definition, all_options, selected):
        ...
        self.columns = urwid.Columns(
            [(60, self.textbox), self.listbox], dividechars=3, focus_column=1
        )

        self.footer = urwid.AttrWrap(urwid.Text(self.footer_text), "foot")
        self.view = urwid.Frame(
            urwid.AttrWrap(self.columns, "body"), footer=self.footer
        )

    def main(self):

        self.loop = urwid.MainLoop(
            self.view, self.palette, unhandled_input=self.unhandled_keypress
        )
        self.loop.run()
...
def get_selection(word, pos):
    ...
    all_ids = selected_ids + [
        x
        for x in sentences.get_all_sentence_ids(word, pos)["ids"]
