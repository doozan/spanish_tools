...
def should_ignore(item):
    if "archaic" in (tag.lower() for tag in item['tags']):
        return True

    if "dated" in (tag.lower() for tag in item['tags']):
        return True

    return False

def do_analysis(items):
    ...
    # Detect nouns that are different as male/female
